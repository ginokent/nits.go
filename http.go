...
// NewMethodsHandler returns convenience functions for registering multiple HTTP methods to (*http.ServeMux).Handle.
// See below for an example of usage:
//
//	func newRouter() *http.ServeMux {
//		router := http.NewServeMux()
//		methods, register := nits.HTTP.NewMethodsHandler(http.NotFoundHandler())
//
//		router.Handle("/echo", methods(register("POST", http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) { io.Copy(rw, r.Body) }))))
//		router.Handle("/hello", methods(register("GET", http.HandlerFunc(func(rw http.ResponseWriter, _ *http.Request) { io.Copy(rw, strings.NewReader("hello")) }))))
//		router.Handle("/status/302", methods(register("POST", http.RedirectHandler("/hello", 302)).Register("GET", http.RedirectHandler("/hello", 302))))
//
//		return router
//	}
